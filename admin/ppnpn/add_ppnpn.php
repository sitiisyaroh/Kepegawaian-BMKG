	...
	<form action="" method="post" enctype="multipart/form-data">
		<div class="card-body">
			...
			<div class="form-group row">
				...
				<div class="col-sm-5">
					<input type="text" class="form-control" id="id_pegawai" name="id_pegawai" placeholder="ID_Pegawai" required>
				</div>
			</div>
...
<?php
    ...
    if (isset ($_POST['Simpan'])){
		...
		if(!empty($sumber)){
        $sql_simpan = "INSERT INTO tb_ppnpn (id_pegawai, nama, jenis_kelamin, tempat_lahir, date, agama, email, alamat, no_hp, status, foto) VALUES (
            '".$_POST['id_pegawai']."',
			'".$_POST['nama']."',
			'".$_POST['jenis_kelamin']."',
			'".$_POST['tempat_lahir']."',
    ...
    if ($query_simpan) {
      ...
      Swal.fire({title: 'Tambah Data Berhasil',text: '',icon: 'success',confirmButtonText: 'OK'
          ...
          window.location = 'index.php?page=data-ppnpn';
          }
      })</script>";
      }else{
      echo "<script>
      Swal.fire({title: 'Tambah Data Gagal',text: '',icon: 'error',confirmButtonText: 'OK'
          ...
          window.location = 'index.php?page=add-ppnpn';
          }
      })</script>";
	}
	}elseif(empty($sumber)){
